...
void update_data(void) {
	os_sprintf(buffer,
			"{\"command\":\"COMMAND_UPDATE\",\"relay_address\":\""MACSTR
			"\",\"devices\":[{\"device_id\":\"0\",\"device_data\":\"%d\",\"device_name\":\""DEVICE0_NAME
			"\",\"device_type\":\""DEVICE0_TYPE
			"\",\"device_icon\":\""DEVICE0_ICON
			"\",\"device_relay\":\""MACSTR"\"}]}\r\n",
...
void tcp_connected(void *arg) {
	...
	os_sprintf(buffer,
			"{\"command\":\"COMMAND_IDENTIFY\",\"relay_address\":\""MACSTR
			"\",\"devices\":[{\"device_id\":\"0\",\"device_data\":\"%d\",\"device_name\":\""DEVICE0_NAME
			"\",\"device_type\":\""DEVICE0_TYPE
			"\",\"device_icon\":\""DEVICE0_ICON
			"\",\"device_relay\":\""MACSTR
			"\"}]}\r\n", MAC2STR(relay_macaddress), wc_bulb_data,
			MAC2STR(relay_macaddress));
	os_printf("Sending: %s\n", buffer);
...
void user_init(void) {
	...
	gpio_output_set(BIT2, 0, BIT2, BIT12);
...
LOCAL void gpio_intr_handler(void) {
	...
	if (gpio_status & BIT(12)) {
		...
		if (auto_mode) {
			...
			os_timer_arm(&auto_light_timer, 30000, 0);
		}
		gpio_pin_intr_state_set(GPIO_ID_PIN(12), GPIO_PIN_INTR_POSEDGE);
	}
}
