...
/**
 * Main entry point. Incoming events from Alexa Lighting APIs are processed via
 * this method.
 */
exports.handler = function(event, context) {
	...
	/**
	 * The namespace of "Discovery" indicates a request is being made to the
	 * lambda for discovering all appliances associated with the customer's
	 * appliance cloud account. can use the accessToken that is made available
	 * as part of the payload to determine the customer.
	 */
	...
	/**
	 * The namespace of "Control" indicates a request is being made to us to
	 * turn a given device on, off or brighten. This message comes with the
	 * "appliance" parameter which indicates the appliance that needs to be
	 * acted on.
	 */
...
/**
 * This method is invoked when we receive a "Discovery" message from Alexa
 * Connected Home Skill. We are expected to respond back with a list of
 * appliances that we have discovered for a given customer.
 */
function handleDiscovery(accessToken, context) {
	...
	/**
	 * Craft the final response back to Alexa Connected Home Skill. This will
	 * include all the discoverd appliances.
	 */
	var payloads = {
...
/**
 * Control events are processed here. This is called when Alexa requests an
 * action (IE turn off appliance).
 */
function handleControl(event, context) {
	if (event.header.namespace === 'Alexa.ConnectedHome.Control') {
		...
		var serverError = function(e) {
			log('Error', e.message);
			context.fail(generateControlError('TurnOnRequest',
